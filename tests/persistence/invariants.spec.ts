...
test.describe("Persistence invariants", () => {
  test("keeps API status, subscription row, invoice, webhook event, and audit aligned after activation", async () => {
  ...
  test("never leaves an active subscription with only failed invoices", async () => {
    const world = TestWorld.start();
  ...
  test("records duplicate webhooks as a single processed event_id", async () => {
    const world = TestWorld.start();
    const created = await world.given.basicTrial();
  ...
  test("isolates data between harness instances (no stale rows)", async () => {

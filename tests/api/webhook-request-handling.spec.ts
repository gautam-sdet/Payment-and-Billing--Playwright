...
test.describe("API contract — webhook request handling", () => {
  test("rejects a missing signature before any business processing", async () => {
  ...
  test("rejects a payload missing event_id", async () => {
    const world = TestWorld.start();
    const created = await world.given.basicTrial();
  ...
  test("accepts a valid signed webhook", async () => {
    const world = TestWorld.start();
    const created = await world.given.basicTrial();

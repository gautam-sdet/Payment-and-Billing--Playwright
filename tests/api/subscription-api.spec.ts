...
test.describe("API contract — subscriptions", () => {
  test("creates a basic subscription in trialing without charging", async () => {
  ...
  test("creates a pro subscription, charges immediately, and returns active", async () => {
    const world = TestWorld.start();
  ...
  test("retrieves an existing subscription", async () => {
    const world = TestWorld.start();
  ...
  test("returns 404 for an unknown subscription", async () => {
    const world = TestWorld.start();
  ...
  test("rejects an unknown plan without calling the provider", async () => {
    const world = TestWorld.start();
  ...
  test("rejects an unknown customer without calling the provider", async () => {
    const world = TestWorld.start();
  ...
  test("rejects a body missing customer_id", async () => {
    const world = TestWorld.start();
  ...
  test("rejects an invalid payment method token", async () => {
    const world = TestWorld.start();
  ...
  test("cancels a trialing subscription via API", async () => {
    const world = TestWorld.start();
    const created = await world.given.basicTrial();
  ...
  test("returns 409 when canceling an already-canceled subscription", async () => {
    const world = TestWorld.start();

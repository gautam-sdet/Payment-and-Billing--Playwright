...
test.describe("End-to-end billing flows", () => {
  test("covers trial → failed charge → retry success, matching API, DB, and provider", async () => {
  ...
  test("covers webhook-driven activation then customer cancel as a terminal state", async () => {
    const world = TestWorld.start();
    const created = await world.given.basicTrial();

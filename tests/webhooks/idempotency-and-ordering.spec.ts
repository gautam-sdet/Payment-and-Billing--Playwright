...
test.describe("Webhooks — idempotency and duplicate delivery", () => {
  test("processes the same event_id exactly once", async () => {
  ...
  test("does not create a duplicate invoice row on redelivery", async () => {
    const world = TestWorld.start();
    const created = await world.given.basicTrial();
    const payload = world.paymentWebhook(created.body.id, {
      eventId: "evt_dup_inv",
...
test.describe("Webhooks — out-of-order / stale delivery", () => {
  test("does not regress active when payment.failed arrives after payment.succeeded for the same invoice", async () => {
    const world = TestWorld.start();
    const created = await world.given.basicTrial();
  ...
  test("applies a late payment.succeeded after payment.failed for the same invoice", async () => {
    const world = TestWorld.start();
    const created = await world.given.basicTrial();
  ...
  test("records a refund without changing lifecycle status", async () => {
    const world = TestWorld.start();

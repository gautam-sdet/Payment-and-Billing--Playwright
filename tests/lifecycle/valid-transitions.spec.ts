...
test.describe("State machine — valid transitions", () => {
  test("trialing -- trial ends, first charge succeeds --> active", async () => {
  ...
  test("trialing -- trial ends, first charge fails --> past_due", async () => {
    const world = TestWorld.start();
    const created = await world.given.basicTrial();
  ...
  test("active -- recurring charge fails --> past_due", async () => {
    const world = TestWorld.start();
  ...
  test("past_due -- retry charge succeeds --> active", async () => {
    const world = TestWorld.start();
  ...
  test("past_due -- retries exhausted --> canceled", async () => {
    const world = TestWorld.start();
    const created = await world.given.basicTrial();
  ...
  test("active -- customer/API cancel --> canceled", async () => {
    const world = TestWorld.start();
    const created = await world.given.proActive();
  ...
  test("trialing -- customer/API cancel --> canceled", async () => {
    const world = TestWorld.start();
    const created = await world.given.basicTrial();
  ...
  test("applies plan price and trial length from the plan strategy", async () => {
    const world = TestWorld.start();
  ...
  test("trialing can also activate from a payment.succeeded webhook", async () => {
    const world = TestWorld.start();
    const created = await world.given.basicTrial();
  ...
  test("trialing -- payment.failed webhook --> past_due without a provider charge", async () => {
    const world = TestWorld.start();
    const created = await world.given.basicTrial();
  ...
  test("active -- payment.failed webhook --> past_due", async () => {
    const world = TestWorld.start();
    const created = await world.given.proActive();
  ...
  test("past_due -- payment.succeeded webhook --> active", async () => {
    const world = TestWorld.start();
    const created = await world.given.pastDueFromTrial();
  ...
  test("active -- recurring charge succeeds --> stays active and records another paid invoice", async () => {
    const world = TestWorld.start();
    const created = await world.given.proActive();
    world.willCharge("succeeded");

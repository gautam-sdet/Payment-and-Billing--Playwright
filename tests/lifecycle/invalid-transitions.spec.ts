...
test.describe("State machine — invalid transitions are impossible", () => {
  test("does not reactivate a canceled subscription via payment.succeeded", async () => {
  ...
  test("does not move canceled -> past_due via payment.failed", async () => {
    const world = TestWorld.start();
    const created = await world.given.canceledTrial();
  ...
  test("refuses to bill a canceled subscription", async () => {
    const world = TestWorld.start();
    const created = await world.given.canceledTrial();

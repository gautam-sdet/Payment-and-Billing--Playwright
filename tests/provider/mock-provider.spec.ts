...
test.describe("Mock payment provider interactions", () => {
  test("is called exactly once per genuine billing attempt with amount, customer, and idempotency key", async () => {
  ...
  test("is not called when creation is rejected", async () => {
    const world = TestWorld.start();
  ...
  test("treats a timeout as a failed charge, not as active", async () => {
    const world = TestWorld.start();
    const created = await world.given.basicTrial();
  ...
  test("treats a decline on pro create as past_due with a failed invoice", async () => {
    const world = TestWorld.start();
  ...
  test("is not called when a webhook is replayed", async () => {
    const world = TestWorld.start();
    const created = await world.given.basicTrial();
